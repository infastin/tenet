...
TrbVector *adj_list_edges(AdjList *self)
{
	trb_return_val_if_fail(self != NULL, NULL);

	TrbVector *result = trb_vector_init(NULL, sizeof(Edge));
	trb_return_val_if_fail(result != NULL, NULL);

	for (usize i = 0; i < self->size; ++i) {
		AdjListVertex *vertex = adj_list_vertex(self, i);
		for (usize j = 0; j < vertex->edges.len; ++j) {
			AdjListNode *node = adj_list_node(vertex, j);
			trb_vector_push_back(result, trb_get_ptr(Edge, vertex->index, node->index, node->dist));
		}
	}
